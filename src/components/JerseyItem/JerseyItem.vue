...
const strokeColor = computed(() => {
  ...
  return isLight(props.color ?? '#D0142C')
    ? 'var(--color-gray-300)'
    : '#FFFFFF' // hardcoded — jersey color is a fixed hex, not a theme token
})

const numberStyle = computed(() => ({
  color: isLight(props.color ?? '#D0142C')
    ? '#161616'  // hardcoded — jersey is user-defined hex, dark mode must not change number color
    : '#FFFFFF', // hardcoded — jersey color doesn't invert with theme
}))
</script>
