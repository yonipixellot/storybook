...
export const DarkMode: Story = {
  ...
  render: () => ({
    components: { AppBadge },
    template: `
      <div style="${box.replace('#E8E8E8', '#444444')}">
        <p style="${title.replace('#161616', '#E8E8E8')}">Dark Mode — All Variants</p>
        <hr style="${hr.replace('#E8E8E8', '#444444')}" />
        <div style="display:flex;gap:16px;align-items:flex-start">
          <div style="text-align:center">
            <AppBadge variant="live" />
            <p style="${stLabel.replace('#979797', '#888888')}">live</p>
          </div>
          <div style="text-align:center">
            <AppBadge variant="premium" />
            <p style="${stLabel.replace('#979797', '#888888')}">premium</p>
          </div>
          <div style="text-align:center">
            <AppBadge variant="free" />
            <p style="${stLabel.replace('#979797', '#888888')}">free</p>
          </div>
          <div style="text-align:center">
            <AppBadge variant="claimed" />
            <p style="${stLabel.replace('#979797', '#888888')}">claimed</p>
          </div>
          <div style="text-align:center">
            <AppBadge variant="highlights" />
            <p style="${stLabel.replace('#979797', '#888888')}">highlights</p>
          </div>
          <div style="text-align:center">
            <AppBadge variant="new" />
            <p style="${stLabel.replace('#979797', '#888888')}">new</p>
          </div>
        </div>
      </div>
    `,
